...
import { startFeaturedCheckout } from "@/app/artist/promote/actions";
...
function PlanRow({ trackId, plan }: { trackId: string; plan: Plan }) {
  ...
  async function onCheckout() {
    setPending(true);
    setError(null);
    const r = await startFeaturedCheckout(trackId, plan.id);
    setPending(false);
    if ("error" in r) {
  ...
  return (
    <div className="flex flex-col gap-3 rounded-lg border border-zinc-200 p-4 dark:border-zinc-800 sm:flex-row sm:flex-wrap sm:items-center sm:justify-between">
